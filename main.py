...
def mutual_information_per_class(probs, conditioning=False, epsilon=0.0001):
    ...
    for value in range(n_classes): # treat each possible class value as binary variable
        mi = 0
        p_f = np.sum(joint_fy, axis=1)[value]
        p_y= np.sum(joint_fy, axis=0)[value]
        p_fy = joint_fy[value, value]
        
        if conditioning == False:
            mi += p_fy * np.log2((p_fy + epsilon) / (p_f * p_y + epsilon))
            mi += (1 - p_fy) * np.log2((1 - p_fy + epsilon) / ((1 - p_f) * (1 - p_y) + epsilon))
        else:
